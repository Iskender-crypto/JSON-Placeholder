<template>
  <div class="container my__container" v-if="loaded">
    <div class="d-flex justify-content-between align-items-center">
      <div>
        <h1 class="title"><b>Альбом</b></h1>
        <div class="breadcrumb">
          <router-link class="breadcrumb__item" to="/">Главная</router-link>>
          <router-link class="breadcrumb__item breadcrumb__actived" to="/albums"
            >Посты</router-link
          >
        </div>
      </div>
    ...
    <app-table :items="items" :columns="columns">
      ...
      >
      <template #UserId="{ item }">{{ item.userId }}</template>
